...
void main() {
  ...
  printCodeList(bytes1);
  var bytes2 = stringToI32List("yjmK14040842a000");
  print("----------------");
  printCodeList(bytes2);

  var keyExpansionResult = keyExpansion([
    // dart format off
    0x2b, 0x7e, 0x15, 0x16,
    0x28, 0xae, 0xd2, 0xa6,
    0xab, 0xf7, 0x15, 0x88,
  ...
  keyExpansionResult.forEach((k) {
    print(toHex(k, hexLenght: 32, header: false));
  });
  print('------------------------');
...
String toHex(int v, {int hexLenght = 8, bool header = false}) {
  String ret = "";
  while (v > 0) {
    ret = HEX[v & 0xf] + ret;
    v >>= 4;
...
void printCodeList(List<int> codeList) {
  codeList.forEach((i) {
    print(toHex(i, hexLenght: 32, header: true));
  });
}

List<int> stringToI32List(String str) {
...
List<int> i8ListToI32List(List<int> i8List) {
...
List<int> stringToI8List(String str) {
  var bytes = str.codeUnits;
  return bytes;
}
...
List<int> cipher(List<int> input, List<int> w) {
  List<int> temp = [0, 0, 0, 0];
  List<int> s = input;
  int round = 0;
  for (int i = 0; i < 4; i++) {
    s[i] ^= w[i];
